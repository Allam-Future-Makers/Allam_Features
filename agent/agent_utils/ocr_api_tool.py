...
class ScanDocFlowOcr:
    def __init__(self, instance):
        ...
        self.access_token = self.instance.ocr_keys[
            self.instance.iterator % len(self.instance.ocr_keys)
        ]
    ...
    def get_text(self, image_path):
        ...
        organized_text = "\n".join(
            [
                " ".join(re.sub(r"\s+", " ", text_line.strip()).split()[::-1])
                for text_line in decoded_text.split("\n")
            ]
        )
